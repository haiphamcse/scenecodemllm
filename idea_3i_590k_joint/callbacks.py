"""In-training VSI-Bench generation metrics (idea_3i_590k: live decode).

Same as idea_3i's callback except frames are decoded from the video path rather
than read from a pre-extracted cache, so eval no longer depends on anything
having been built first. The user turn carries the VGGT placeholder blocks and
``raw_videos`` is passed at generation time, so the Perceiver scatter is active
at eval exactly as in training.
"""
...
from collator import extract_raw_video_tensors, load_frames, task_messages
        ...
        if self.state.is_main_process:
            ...
            with torch.no_grad():
                for idx in tqdm(
                    ...
                    # VGGT gets its OWN balanced-512 copy, not the MLLM's tensors --
                    # the same split the joint collator makes at training time. Passing
                    # video_inputs here (as the idea_3i_590k callback did) would feed the
                    # encoder a different resolution at eval than it saw in training.
                    inputs["raw_videos"] = [
                        v.to(device) for v in
                        extract_raw_video_tensors(frames, self.vggt_image_resolution)]
