...
Stored per clip (fp32, as exported), MULTISCALE:

    patch_<i> [T, Np, vggt_dim]   VGGT patch tokens of aggregator layer VGGT_LAYERS[i]
    camera    [T, 17, vggt_dim]   camera token + 16 register tokens, deepest layer only

The layer list is part of the params, so this cache cannot be confused with the
single-scale vggt_cache_256_64f (same keying otherwise).
"""
...
# The aggregator's cached_layer_indices (4, 11, 17, 23), shallow to deep.
...
def save(cache_root, key: str, patches: list[torch.Tensor], camera: torch.Tensor,
         params: Dict[str, Any]) -> Path:
    """Write one clip's tokens (one patch tensor per scale). Atomic: build beside the target, then rename in."""
    if len(patches) != len(params["vggt_layers"]):
        raise ValueError(f"got {len(patches)} patch tensors for layers {params['vggt_layers']}")
    ...
    save_file(
        {**{f"patch_{i}": p.contiguous() for i, p in enumerate(patches)},
...
def load(cache_root, key: str, params: Dict[str, Any]) -> tuple[list[torch.Tensor], torch.Tensor]:
    """``(S x patch [T, Np, d], camera [T, 17, d])`` or raise. Never runs VGGT."""
    path = cache_path(cache_root, key)
    if not path.exists():
        raise VggtCacheMiss(
    ...
    return [tensors[f"patch_{i}"] for i in range(len(params["vggt_layers"]))], tensors["camera"]


def params_from(vggt_image_resolution: int, video_fps: float, video_max_frames: int,
                vggt_checkpoint: str, vggt_layers=VGGT_LAYERS) -> Dict[str, Any]:
    ...
    return {
        ...
        "vggt_layers": [int(l) for l in vggt_layers],
    }
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        patches = [torch.randn(2, 4, 8) for _ in VGGT_LAYERS]
        camera = torch.randn(2, 17, 8)
        assert not is_fresh(d, clip_key(vqa), params)
        save(d, clip_key(vqa), patches, camera, params)
        assert is_fresh(d, clip_key(vqa), params)

        got_patches, got_camera = load(d, clip_key(vqa), params)
        assert len(got_patches) == 4 and torch.equal(got_camera, camera)
        assert all(torch.equal(g, p) for g, p in zip(got_patches, patches))
        assert got_patches[0].dtype == torch.float32
        # The single-scale cache's params (no layer list) must not read this cache.
        assert not is_fresh(d, clip_key(vqa), {**params, "vggt_layers": [23]})
