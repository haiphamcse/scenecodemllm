...
def load_vggt(checkpoint: str, device: str, dtype: torch.dtype):
    ...
    if sorted(aggregator.cached_layer_indices) != vggt_cache.VGGT_LAYERS:
        raise ValueError(f"aggregator caches {sorted(aggregator.cached_layer_indices)}, "
                         f"cache expects {vggt_cache.VGGT_LAYERS}")
    return aggregator


@torch.no_grad()
def encode(aggregator, frames, resolution: int, device: str, dtype: torch.dtype):
    """Frames -> ``(S x patch [T, Np, d], camera [T, 17, d])`` in fp32, ready to store.

    The aggregator fills only its cached_layer_indices (4, 11, 17, 23), the rest are
    None; every cached depth is kept (multiscale), camera tokens from the deepest only.
    """
    ...
    scales = [t[0] for t in tokens_list if t is not None]  # each [T, 17 + Np, d]; batch is 1 clip
    camera = scales[-1][:, :patch_token_start, :]
    patches = [t[:, patch_token_start:, :] for t in scales]
    ...
    return [p.float().cpu() for p in patches], camera.float().cpu()
...
def main() -> int:
    ...
    p.add_argument("--max_clips", type=int, default=None, help="Smoke: export at most this many clips of the shard.")
    ...
    for i, (key, row) in enumerate(todo, 1):
        try:
            ...
            patches, camera = encode(aggregator, frames, args.vggt_image_resolution,
                                     args.device, dtype)
            store = {"fp32": torch.float32, "bf16": torch.bfloat16}[args.store_dtype]
            out = vggt_cache.save(args.cache_root, key, [p.to(store) for p in patches],
                                  camera.to(store), params)
        except Exception as exc:  # one bad clip must not sink the export
            failed += 1
            print(f"FAILED {key}: {type(exc).__name__}: {exc}", flush=True)
            continue
        total_bytes += out.stat().st_size
        print(f"  [{i}/{len(todo)}] {key} {len(patches)}x patch{tuple(patches[0].shape)} camera{tuple(camera.shape)} "
