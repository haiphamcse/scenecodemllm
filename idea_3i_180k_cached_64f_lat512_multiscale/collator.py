...
come from vggt_cache, exported once by export_vggt_features.py (multiscale: one patch
tensor per cached VGGT depth per clip). A missing entry raises;
...
def make_collator(
    ...
    vggt_token_dtype: torch.dtype = torch.bfloat16,
    ...
    def collate_fn(examples: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for example in examples:
            ...
            # Cast to the training dtype HERE, in the worker. The model casts to
            # inputs_embeds.dtype anyway, so the Perceiver sees the same values; but the
            # Trainer parks the whole accumulation window on the host (see train.py
            # get_train_dataloader), and 4 fp32 scales are ~480 MB/sample: 2 ranks x 128
            # samples blew the 240 GB cgroup (jobs 2104163..2105691, host OOM at step 1).
            patch_tokens.append([p.to(vggt_token_dtype) for p in patch])
            camera_tokens.append(camera.to(vggt_token_dtype))
...
if __name__ == "__main__":
    ...
    collate = make_collator(
        ...
        frame_placeholder_text="<|quad_start|>" * (256 * len(vggt_cache.VGGT_LAYERS)),
        camera_placeholder_text="<|quad_end|>" * 32,
    )
    for task, row in picked.items():
        batch = collate([row])
        patches, camera = batch["vggt_patch_tokens"][0], batch["vggt_camera_tokens"][0]
        assert len(patches) == len(vggt_cache.VGGT_LAYERS), (task, len(patches))
        patch = patches[0]
        ...
        print(f"{task}: {len(patches)}x patch {tuple(patch.shape)} camera {tuple(camera.shape)}, "
