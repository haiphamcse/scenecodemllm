...
def collect_video_paths(data_root: Path, jsonl_path: Path, hf_home: str | None) -> list[str]:
    """Resolved absolute paths of every train (scannetpp) + eval video."""
    paths: set[str] = set()

    for record in filter_train_records(jsonl_path):
...
def main() -> None:
    ...
    video_paths = collect_video_paths(data_root, Path(args.jsonl_path), args.hf_home)
    logging.info(
        "Videos to extract: %d -> %s (fps=%s max_frames=%d)",
        len(video_paths), cache_root, args.video_fps, args.max_frames,
    )
