...
Frames are read by the SAME collator function the trainer calls (load_frames -> the clip's
det_es JPEGs), then extract_raw_video_tensors, so T == the det_es frame count and the
cache entry is stamped frames=det_es (vggt_cache.params_from).
