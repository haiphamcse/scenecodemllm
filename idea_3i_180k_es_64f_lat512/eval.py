...
# This fork's collator reads det_es frames (training only). Eval must sample the
# VSI-Bench clip from the video like every other experiment, so load_frames comes
# from the main fork's collator (same pixel budget, live decode).
_spec_c = importlib.util.spec_from_file_location(
    "collator_live", _FINETUNING_ROOT / "idea_3i_180k_cached_64f_lat512" / "collator.py")
_collator_live = importlib.util.module_from_spec(_spec_c)
_spec_c.loader.exec_module(_collator_live)
load_frames = _collator_live.load_frames
