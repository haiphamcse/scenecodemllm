"""Standalone VSI-Bench evaluation for idea_3i (Qwen3-VL + VGGT-Perceiver scatter).

Same structure as idea_3a_cached/eval.py, plus the idea_3i specifics: the user
turn carries the ``<|quad_start|>``/``<|quad_end|>`` placeholder blocks and
``raw_videos`` is handed to ``generate`` so the VGGT-Perceiver scatter is active.
...
idea_3i_tuned was trained with 256 frame latents and widening factor 2.

  conda activate vsibench_eval_full
  cd spatial_reasoning/finetuning
  python idea_3i/eval.py --adapter_path results/idea_3i_tuned/checkpoint-1800 \
      --split full --frame_num_latents 256 --frame_widening_factor 2
"""
...
# _THIS_DIR first so the local (idea_3i_590k) collator shadows the common one.
    ...
    with preds_path.open("w", encoding="utf-8") as preds_file:
        for idx in tqdm(order, desc=f"VSI-Bench ({args.split})", unit="sample"):
            ...
            messages = user_only_messages(
                example_video_path(example, data_root), example,
                frame_placeholder_text, camera_placeholder_text,
                args.video_fps, args.video_max_frames,
            )
