...
# idea_3i_180k_cached_64f_lat512_multiscale: the lat512 launcher + the legacy multiscale idea
# (idea_3i_590k_join_multiscale): VGGT-Omega aggregator layers 4/11/17/23 from the MULTISCALE
# cache vggt_cache_256_64f_ms, ONE shared frame Perceiver per depth, latents concatenated:
# frame placeholders = 256 latents x 4 scales = 1024 (+32 camera, deepest layer only).
# Everything else identical to jz_train_180k_lat512.sh.
#
...
OUTPUT_DIR="${SR_OUTPUT_ROOT:-results}/idea_3i_180k_cached_64f_lat512_multiscale"
CACHE_ROOT="${SR_VGGT_CACHE:-${SCRATCH:-/home/ducpham/scratch/Working}/dataset/vggt_cache_256_64f_ms}"
MIX_JSONL="${SR_MIX_180K:-${SCRATCH:-/home/ducpham/scratch/Working}/dataset/vgllm_data/joint180k_vlm3r/vqa_train.jsonl}"
FRAME_CACHE="${SR_FRAME_CACHE:-${SCRATCH:-/home/ducpham/scratch/Working}/dataset/vgllm_data/joint130k/frame_cache_64f}"
...
if [[ ! -f "${MIX_JSONL}" ]]; then
  echo "ERROR: ${MIX_JSONL} missing. Run idea_3i_180k_cached_64f_lat512_multiscale/build_mix_180k.py first." >&2
  exit 1
fi
[[ -d "${CACHE_ROOT}" ]] || { echo "ERROR: ${CACHE_ROOT} missing. Run scripts/idea_3i_180k_cached_64f_lat512_multiscale/export_vggt_64f_ms_v100.slurm first." >&2; exit 1; }
[[ -d "${FRAME_CACHE}" ]] || { echo "ERROR: ${FRAME_CACHE} missing." >&2; exit 1; }
...
args=(
  ...
  --learning_rate 1e-4
  # PER SCALE: 256 x 4 cached VGGT depths = 1024 frame placeholders.
  ...
  # 4 workers x prefetch 1 here (was 8 x 2): each in-flight sample is ~240 MB of bf16
  # multiscale tokens, and the host cgroup (240 GB at 2 GPUs) is what the accumulation
  # window already fills. Loader is JPEG + one 480 MB safetensors read per sample.
  --dataloader_num_workers 4
  --dataloader_prefetch_factor 1

  # Pre-decoded frames for the Qwen branch (vqa rows; det rows are JPEG dirs already).
...
accelerate launch --config_file common/multi_gpu.yaml \
  ...
  idea_3i_180k_cached_64f_lat512_multiscale/train.py "${args[@]}" "$@"
