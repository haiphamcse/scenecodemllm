...
# idea_3a_180k_cached_64f: plain Qwen3-VL-2B LoRA SFT (no VGGT, no Perceiver, no placeholder
# tokens) on the SAME VQA-only 180k mix as jz_train_180k_lat512.sh (VSI-590K 110,944 rows +
# VLM-3R 68,548 rows, joint180k_vlm3r/vqa_train.jsonl, ~702 steps/epoch at GLOBAL_BATCH 256),
# same 64 frames at 128 tok/frame from joint130k/frame_cache_64f, same LoRA r64 / lr 1e-4 /
# cosine / GB 256. The no-3D-context control for the lat512 run.
...
OUTPUT_DIR="${SR_OUTPUT_ROOT:-results}/idea_3a_180k_cached_64f"
...
if [[ ! -f "${MIX_JSONL}" ]]; then
  echo "ERROR: ${MIX_JSONL} missing. Run idea_3a_180k_cached_64f/build_mix_180k.py first." >&2
  exit 1
fi
...
args=(
  ...
  # Liger's RMSNorm/RoPE/SwiGLU kernels, monkey-patched onto the plain Qwen3-VL by TRL.
  ...
  --ddp_timeout 7200
  --eval_strategy no
  # EXPLICIT False: the dataclass default is True with eval_steps 20.
...
accelerate launch --config_file common/multi_gpu.yaml \
  ...
  idea_3a_180k_cached_64f/train.py "${args[@]}" "$@"
