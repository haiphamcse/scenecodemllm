...
OUTPUT_DIR="results/idea_3i_spatialstack"
...
# variable vs the Perceiver runs: same global batch 64, lr 1e-4, cosine, 1 epoch.
#
...
#                         gpu013 the pair is `NV4` (NVLink), so leave P2P on.
...
#   perceiver_lr 3e-4     -> dropped. Single lr 1e-4 for mergers and LoRA alike.
...
accelerate launch --config_file common/multi_gpu.yaml --num_processes "${NPROC_PER_NODE}" --main_process_port 0 idea_3i_spatialstack/train.py "${args[@]}" "$@"
