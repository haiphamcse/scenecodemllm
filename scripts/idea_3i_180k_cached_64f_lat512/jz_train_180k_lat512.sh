...
OUTPUT_DIR="${SR_OUTPUT_ROOT:-results}/idea_3i_180k_cached_64f_lat512"
CACHE_ROOT="${SR_VGGT_CACHE:-${SCRATCH:-/home/ducpham/scratch/Working}/dataset/vggt_cache_256_64f}"
...
if [[ ! -f "${MIX_JSONL}" ]]; then
  echo "ERROR: ${MIX_JSONL} missing. Run idea_3i_180k_cached_64f_lat512/build_mix_180k.py first." >&2
  exit 1
fi
[[ -d "${CACHE_ROOT}" ]] || { echo "ERROR: ${CACHE_ROOT} missing. Run scripts/idea_3i_130k_joint_cached_64f/export_vggt_64f_v100.slurm first." >&2; exit 1; }
...
accelerate launch --config_file common/multi_gpu.yaml \
  ...
  idea_3i_180k_cached_64f_lat512/train.py "${args[@]}" "$@"
