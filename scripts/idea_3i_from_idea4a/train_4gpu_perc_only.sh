...
# 4-GPU idea_3i run that transfers ONLY the perceiver from idea_4a. The LLM is the stock
# Qwen3-VL-2B-Instruct -- no idea_4a LoRA merged into it.
#
# train_4gpu.sh            -> merged LLM (noise ckpt-10000) + its projector
# train_4gpu_scratch1400.sh-> merged LLM (scratch ckpt-1400) + its projector
# this script              -> STOCK LLM + the scratch ckpt-1400 projector
#
# So this isolates the transfer to the perceiver: whatever it gains over a random-init
# perceiver is attributable to idea_4a's detection pretraining alone, with none of the
# LLM-side adaptation that merging the adapter also carries over.
#
# --lora_enable stays ON. "Original LLM weight" means nothing is merged in up front, not
# that the LLM is frozen -- a fresh r64/alpha128 adapter still trains during this run.
#
# BASE_MODEL is the stock checkpoint, but the projector still comes out of the merged dir:
# merge_lora.py writes vggt_projector.safetensors there as a standalone file, and it is
# independent of whether the LoRA half was ever applied to any LLM.
#
# Separate OUTPUT_DIR on purpose. Sharing one with the other scripts would let
# --resume_from_checkpoint latest pick up a checkpoint trained from a different init.
#
...
PROJECTOR="${PROJECTOR:-results/idea_4a_video_scratch_merged1400/vggt_projector.safetensors}"
OUTPUT_DIR="results/idea_3i_perc_only_scratch1400"
...
args=(
  ...
  --learning_rate 1e-4
  # 5e-5, matching the other two scripts so LR is not a confound across the three lineages.
  # The original rationale (don't let the projector outrun the LLM weights it was matched to)
  # does NOT apply here -- the LLM is stock, so the projector has to re-match a model it has
  # never seen, which would argue for 1e-4 or higher. Held at 5e-5 for comparability.
  --perceiver_lr 5e-5
