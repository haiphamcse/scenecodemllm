...
# this script     -> idea_4a_video_scratch_b128_lora/checkpoint-1400 (perceiver from random
#                    init, global batch 128, 1400 of 3381 steps: loss 0.341, tok_acc 0.867)
#
...
#       --adapter results/idea_4a_video_scratch_b128_lora/checkpoint-1400 \
#       --out     results/idea_4a_video_scratch_merged1400
#
...
MERGED_MODEL="${MERGED_MODEL:-results/idea_4a_video_scratch_merged1400}"
OUTPUT_DIR="results/idea_3i_from_idea4a_scratch1400"
...
args=(
  ...
  --learning_rate 1e-4
  # 5e-5, matching train_4gpu.sh so the two lineages differ only in their init. Note the
  # argument for it is weaker here: this projector has 1400 steps behind it, not ~10k, so it
  # is less settled and could arguably take the full 1e-4. Held at 5e-5 for comparability.
  --perceiver_lr 5e-5
