...
class SceneGraphReconCallback(TrainerCallback):
    ...
    def _prep_inputs(self, messages, frames, device):
        prompt = self.processor.apply_chat_template(
            messages, tokenize=False, add_generation_prompt=True
        )
        # VGGT's balanced resize, MLLM's budgeted copy -- the same two the collator
        # builds. Eval must feed the video too, or it would score the model on a prompt
        # shape it never trained on.
        video_inputs = extract_raw_video_tensors(frames, self.vggt_image_resolution)
        ...
        if self.state.is_main_process:
            ...
            with torch.no_grad():
                for idx in tqdm(
                    ...
                    frames = load_example_video(example, self.data_root, self.cache_root)
                    messages = graph_only_messages(
                        frames,
                        self.frame_placeholder_text, self.camera_placeholder_text,
                    )
                    inputs = self._prep_inputs(messages, frames, device)
                    if inputs is None:
