"""Qwen3-VL SFT with VLM-3R cross-attention VGGT fusion (idea_3i_vlm3r).

Qwen's video embeddings cross-attend to VGGTOmega's (frozen) camera + patch tokens
and are rewritten in place by the residual, VLM-3R Spatial-Visual-View style (see
model_with_vggt.py). Video frames are read from the
pre-extracted cache shared with idea_3i (run idea_3i/pre_extract_videos.py first).
Trainable: the fusion block (``vggt_projector``) + LoRA adapters on the LLM.
...
accuracy toward 1.0.
  ...
  bash scripts/idea_3i_vlm3r/overfit.sh
"""
...
class VggtFusionArguments:
    """VGGT encoder + VLM-3R cross-attention fusion settings."""
    ...
    geometry_encoder_layer: int = field(
        default=23,
        metadata={"help": "VGGT aggregator layer to read (only 4/11/17/23 are cached)."},
    )
    fusion_num_heads: int = field(
        default=16,
        metadata={"help": "Cross-attention heads. Must divide the text hidden size (2048)."},
    )
    fusion_dropout: float = field(default=0.1, metadata={"help": "Dropout on the fused residual."})
    fusion_lr: Optional[float] = field(
        default=None,
        metadata={"help": "Separate LR for the fusion block (vggt_projector). None = --learning_rate."},
    )
...
def main() -> None:
    ...
    model.initialize_vggt(
        ...
        geometry_encoder_layer=vggt_args.geometry_encoder_layer,
        fusion_num_heads=vggt_args.fusion_num_heads,
        fusion_dropout=vggt_args.fusion_dropout,
    )
    logging.info(
        "VLM-3R cross-attention fusion: VGGT layer %s, %s heads",
        vggt_args.geometry_encoder_layer, vggt_args.fusion_num_heads,
    )
