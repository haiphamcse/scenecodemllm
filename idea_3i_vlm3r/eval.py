"""Standalone VSI-Bench evaluation for idea_3i_vlm3r (VLM-3R cross-attention fusion).

Fork of idea_3i_spatialstack/eval.py: Qwen's video embeddings cross-attend to
VGGT's camera + patch tokens, so the prompt is plain video + text and
``raw_videos`` is handed to ``generate`` to drive the fusion.
...
``--geometry_encoder_layer``/``--fusion_num_heads`` must match the trained
checkpoint, otherwise the loaded vggt_projector shapes will not line up.
  ...
  python idea_3i_vlm3r/eval.py \
      --adapter_path results/idea_3i_vlm3r/checkpoint-2000 --split full
"""
...
def load_model(args: argparse.Namespace, processor: AutoProcessor, dtype: torch.dtype):
    ...
    model.initialize_vggt(
        ...
        geometry_encoder_layer=args.geometry_encoder_layer,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="VSI-Bench eval for idea_3i_vlm3r (VLM-3R cross-attention fusion)."
    )
    ...
    parser.add_argument("--geometry_encoder_layer", type=int, default=23)
