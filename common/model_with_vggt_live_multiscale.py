...
- VGGTOmega (frozen) runs on ``raw_videos`` ([T, C, H, W] in [0, 1], resized and
  normalised by collator.extract_raw_video_tensors) and yields
...
class Qwen3VLWithVggtModel(Qwen3VLModel):
    ...
    def initialize_vggt(
        ...
        # enable_alignment only decides whether the module tree has a text_alignment_head
        # for load_state_dict to fill: the text-aligned checkpoint carries those weights and
        # a plain VGGTOmega() rejects them. Only the aggregator is kept below, so the head is
        # built and immediately dropped -- what actually differs between the two checkpoints
        # is the aggregator that text alignment trained.
        vggt = VGGTOmega(enable_alignment=enable_alignment)
        state_dict = torch.load(checkpoint_path, map_location=self.device)
        vggt.load_state_dict(state_dict)
        del state_dict

        self.vggt_model = vggt.aggregator.to(device=self.device, dtype=self.dtype)
        n_cached = len(self.vggt_model.cached_layer_indices)
        if n_cached != frame_num_scales:
            raise ValueError(
                f"frame_num_scales={frame_num_scales} but the aggregator caches {n_cached} "
                f"layers ({sorted(self.vggt_model.cached_layer_indices)}). The placeholder "
                f"text is sized from frame_num_scales, so a mismatch would only surface as a "
                f"scatter-count error mid-step."
            )
        self.vggt_model.eval()
    ...
    def _extract_vggt_tokens(
        ...
        """Frozen VGGT forward -> ``(camera_tokens [1, T, 17, d], S x patch_tokens [1, T, Np, d])``.

        The aggregator returns one entry per block but only fills the depths in its
        ``cached_layer_indices`` (4, 11, 17, 23); the rest are ``None``. The single-scale
        variant took ``[-1]`` and dropped the other three -- keeping them costs no extra
        VGGT compute, they are already materialised.
        """
        if raw_video.ndim != 4:
            raise ValueError(f"Expected raw video [T, C, H, W], got shape {tuple(raw_video.shape)}")

        video = raw_video.to(device=device, dtype=self.dtype).unsqueeze(0)  # [1, T, C, H, W]

        aggregated_tokens_list, patch_token_start = self.vggt_model(video)
        scales = [t for t in aggregated_tokens_list if t is not None]  # each [1, T, 17 + Np, d]
        # Camera/register tokens stay single-scale: only the deepest cached layer is used.
        camera_tokens = scales[-1][:, :, :patch_token_start, :]
        patch_tokens = [t[:, :, patch_token_start:, :] for t in scales]
    ...
    def _extract_vggt_latents(
        ...
        for video in raw_videos:
            ...
            patch_tokens = [t.flatten(1, 2).to(dtype=dtype) for t in patch_tokens]  # S x [1, T*Np, d]
...
class Qwen3VLWithVggtForConditionalGeneration(Qwen3VLForConditionalGeneration):
    ...
    def initialize_vggt(
        ...
        self.model.initialize_vggt(
            ...
            frame_widening_factor=frame_widening_factor,
            frame_num_scales=frame_num_scales,
...
if __name__ == "__main__":
    ...
    model.initialize_vggt(
        ...
        frame_num_scales=args.frame_num_scales,
    ...
    # VGGT wants its own preprocessing, not qwen's decoded frames: same path the
    # collator takes, which is also what puts the clip in [0, 1].
    from collator import extract_raw_video_tensors
    from torchvision.transforms.functional import to_pil_image

    vggt_videos = extract_raw_video_tensors(
        [to_pil_image(f.to(torch.uint8)) for f in video_tensors[0]]
    )
    model_inputs["raw_videos"] = [t.to(args.device) for t in vggt_videos]
    with torch.no_grad():
