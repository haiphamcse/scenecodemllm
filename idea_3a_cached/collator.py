"""Plain Qwen-VL supervised collator (idea_3a_cached) reading frame caches.

Fork of common/collator.py: same prompt-masking logic and the *same* message
shape (video + question text, no auxiliary 3D placeholders), but instead of
pointing the chat message at a video *path* (which triggers a decode + resize on
every epoch) it loads the pre-extracted resized uint8 frames from the cache (see
idea_3i/pre_extract_videos.py, run at 64 frames) and feeds them as a
``list[PIL.Image]``. That routes ``fetch_video`` through its list branch with no
video decoding.
...
vs the decode path; since VSI-Bench eval originally decodes live, the collator
overwrites the fabricated metadata with the *real* decode metadata stored in the
cache sidecar, so ``_calculate_timestamps`` produces identical timestamps.

Unlike idea_3i this experiment has no VGGT/Perceiver, so there are no placeholder
blocks and no ``raw_videos`` scatter -- it is the plain Qwen3-VL baseline, just
served from cache at 64 frames.
"""
...
def cache_available(example: Dict[str, Any], data_root: Path, cache_root: Path) -> bool:
    """Whether both cache files exist for this example's video."""
...
def load_example_video(example: Dict[str, Any], data_root: Path, cache_root: Path):
...
def training_messages(
    frames, meta: Dict[str, Any], example: Dict[str, Any]
) -> List[Dict[str, Any]]:
    return [
