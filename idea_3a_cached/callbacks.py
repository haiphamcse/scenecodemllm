"""In-training VSI-Bench generation metrics (idea_3a_cached: plain + frame cache).

Fork of common/callbacks.py that reads video frames from the pre-extracted cache
instead of decoding live. Same plain Qwen3-VL message shape as idea_3a (no VGGT
placeholders, no ``raw_videos``); the only change is where the frames come from
and overwriting the list-branch's fabricated metadata with the real decode
metadata stored in the cache sidecar so timestamps match.
"""
